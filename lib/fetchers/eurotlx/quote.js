...
function fetcher(isin, callback) {
  ...
  baseFetcher(url, function(err, response, body) {
    ...
    value = value.substr(value.indexOf(QUOTE_BEGIN) + QUOTE_BEGIN.length + 15, 20);
    value = value.substring(0, value.indexOf(QUOTE_END));

    // Fall back to previous EOD quote
    if (_.isEmpty(value) || value === '-') {
      value = body;
      
      value = value.substr(value.indexOf(EOD_QUOTE_BEGIN) + EOD_QUOTE_BEGIN.length + 15, 20);
      value = value.substr(0, value.indexOf(EOD_QUOTE_END));
    }
